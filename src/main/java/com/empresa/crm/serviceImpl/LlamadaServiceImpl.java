...
public class LlamadaServiceImpl implements LlamadaService {
	...
	public List<Llamada> findByFecha(LocalDate fecha) {
		...
		return repo.findByEmpresaAndFechaBetween(TenantContext.get(), inicio, fin);
	}
	...
	public List<EventoCalendarioDTO> getEventosCalendarioByEmpresa(String empresa) {
		...
		return repo.findByEmpresa(emp).stream().filter(l -> l.getFecha() != null).map(l -> {
	...
	public List<EventoCalendarioDTO> getEventosCalendario() {
		...
		return repo.findByEmpresa(TenantContext.get()).stream().filter(l -> l.getFecha() != null).map(l -> {
			EventoCalendarioDTO dto = new EventoCalendarioDTO();
			dto.setId(l.getId());
			dto.setTitle(l.getMotivo());
	...
	public List<Llamada> getProximasPendientesByEmpresa(String empresa, int limit) {
		...
		return repo.findByEmpresaAndEstadoAndFechaAfterOrderByFechaAsc(empresa, "pendiente", ahora,
	...
	public void pasarPendientesVencidasAHoy(String empresa) {
		...
		String emp = empresa.trim().toUpperCase();
		LocalDateTime ahora = LocalDateTime.now();
		LocalDateTime hoyMismaHora = ahora;

		List<Llamada> vencidas = repo.findByEmpresaAndEstadoAndFechaBefore(emp, "pendiente",
				ahora.toLocalDate().atStartOfDay());

		for (Llamada l : vencidas) {
			LocalDateTime nuevaFecha = l.getFecha().withYear(ahora.getYear()).withMonth(ahora.getMonthValue())
					.withDayOfMonth(ahora.getDayOfMonth());

			l.setFecha(nuevaFecha);
			l.setRecordatorioEnviado(false);
		}
